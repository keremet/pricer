...
<!--?if($_FILES['user_img']){
	...
	if(!is_array(LoadFile('user_img', $white_list, 1048576, $_SERVER['DOCUMENT_ROOT'].'/'.$GLOBALS['site_settings']['site_folder'].$GLOBALS['site_settings']['img_path']))){
		$query = "INSERT INTO images (path,alt,title,creator) VALUES ({?},{?},{?},{?})";
		...
		if($image_id){
			$query = "SELECT id FROM user_images WHERE user = {?} AND main = {?}";
			...
			$query = "INSERT INTO user_images (user,image,alt,title,main,creator) VALUES ({?},{?},{?},{?},{?},{?})";
...
//		$my_img = Img_Select(array('user' => $_SESSION['user_id'], 'main' => 1, array('path')));
		$stmt = $db->prepare("SELECT name, login, text, email FROM users where id = ?");
		...
		<a href="my_purchases.php">Покупки по данным из чеков</a><br>
		<a href="my_purchases_csv.php">Покупки CSV</a><br>
