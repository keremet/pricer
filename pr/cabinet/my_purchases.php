...
<?php
	...
	$stmt = $db->prepare(
        "SELECT r.id, DATE_FORMAT(r.dateTime, '%d-%m-%Y %H:%i:%s') dt, r.userInn, r.user, r.retailPlaceAddress, i.name, i.price, i.quantity
	     FROM receipt r 
	...
	while ($row = $stmt->fetch()) {
		oftTable::row(array('<a href=../receipts/receipt.php?id='.$row['id'].'>'.$row['dt'].'</a>', $row['userInn'], $row['user'], $row['retailPlaceAddress'], $row['name'], money_out($row['price']), $row['quantity']));
	}
