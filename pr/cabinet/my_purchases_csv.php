<?php 
	...
	$stmt = $db->prepare(
        "SELECT DATE_FORMAT(r.dateTime, '%d.%m.%Y %H:%i:%s') dt, r.fiscalDriveNumber, s.name s_name, s.address, r.user, r.retailPlaceAddress, p.name p_name, e.name e_name, p.in_box, i.name, i.price, i.quantity
	     FROM receipt r 
	     LEFT JOIN fdn_to_shop f2s ON f2s.fiscalDriveNumber = r.fiscalDriveNumber
	     LEFT JOIN shops s ON s.id = f2s.shop_id
		 JOIN receipt_item i ON i.receipt_id = r.id
		 LEFT JOIN receipt_item_name_to_product in2p ON in2p.name = i.name
		 LEFT JOIN products p ON p.id = in2p.product_id
