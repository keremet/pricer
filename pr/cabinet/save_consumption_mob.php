<html>
<head>
	<meta charset="utf-8">
</head>
<body>
<?
include('../template/connect.php');
	
if(!$_REQUEST['clsf_id'])
	die('Не указан расход');

if(!$_REQUEST['price'])
	die('Не введена цена');

$price = str_replace(',', '.', $_REQUEST['price']);

if(!is_numeric($price))
	die('Неправильный формат цены');

$pieces = explode("/", $_REQUEST['date_buy']);
$date_buy = $pieces[2].'-'.$pieces[1].'-'.$pieces[0];

$stmt = $db->prepare("INSERT INTO pr_consumption (clsf_id,price,date_buy) VALUES (?,?,?)"); 
if(!$stmt->execute(array($_REQUEST['clsf_id'], $price, $date_buy))){
	print_r($stmt->errorInfo()); echo '<br>';
	die('Ошибка при добавлении цены');
}
echo 'Цена успешно добавлена';

?>
<br><a href=profile_mob.php>На главный классификатор</a>
</body>
</html>
