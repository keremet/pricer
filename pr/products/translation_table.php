...
if(isset($_POST['operation']))
{
	...
	if($op == 'get') {
		...
		$stmt = $db->prepare('SELECT `a`.`id` AS `id`, 
						   ...
						   `b`.`name` AS `cl_name`
					FROM `pr_ids` `a`
					JOIN `pr_consumption_clsf` `b` ON `a`.`prod_id` = `b`.`id`
					WHERE ' . ($id ? "`a`.`prod_id` = ?" : '1'));
		$params = array();
		if($id)
			$params[] = $id;
		...
		if(isset($_POST['id'])) {
			$id = $_POST['id'];
			$stmt = $db->prepare('select `pr_get_product_path`(?) AS `path`');
		...
		if(isset($_POST['id'])) {
			...
			$id = $_POST['id'];
			$stmt = $db->prepare('DELETE FROM `pr_ids` WHERE id=?');
		...
		if(isset($_POST['id']) && isset($_POST['name']) && isset($_POST['inn'])) {
			...
			$stmt = $db->prepare('INSERT INTO `pr_ids` (`prod_id`, `name`, `inn`) VALUES (?, ?, ?)');
